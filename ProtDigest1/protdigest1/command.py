...
# symbols which are imported by "from protdigest1.command import *"
...
from protdigest1.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse("protdigest1"),
                "config/default.cfg")
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename="protdigest1.config")
...
def bootstrap():
    ...
    By default this runs the function 'protdigest1.model.bootstrap_model', which
    ...
    easy_install which will run this function:
        ...
        'bootstrap-protdigest1 = protdigest1.command:bootstrap',
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="ProtDigest1 %s" % version)
    ...
    _read_config(args)
    from protdigest1.model import bootstrap_model
